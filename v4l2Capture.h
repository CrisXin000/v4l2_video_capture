...
#ifndef VIDEO_CAPTURE_V4L2CAPTURE_H
#define VIDEO_CAPTURE_V4L2CAPTURE_H
...
class V4L2Capture {
public:
    V4L2Capture(char *devName, int width, int height, int format);
    virtual ~V4L2Capture();
...
private:
    ...
    int capW;
    int capH;
    int fd_cam;// read video success or not
    cam_buffer *buffers;
...
V4L2Capture::V4L2Capture(char *devName, int width, int height, int format) {
...
int V4L2Capture::initDevice() {
    ...
    cam_format.fmt.pix.pixelformat = V4L2_PIX_FMT_YUV422M;		//要和摄像头支持的类型对应
    ...
    if(ioctl(fd_cam,VIDIOC_G_PARM,&Stream_Parm)==-1)
...
#endif //VIDEO_CAPTURE_V4L2CAPTURE_H
